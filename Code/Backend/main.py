...
def loadGraph():
    ...
    return g, gTrans, frwdTrans, inverTrans

def transformCoords(x, y, trans):
    return trans.transform(x,y)

def findMax(i):
    return max(node[i] for node in g.nodes)+1

def coordToNode(est: float, nth: float, maxRow, maxCol):
...
def findRoute(src: str, dst: str): #src and dst would be strings like: src=lon,lat&dst=long,lat
    try:
        ...
        srcest, srcnth = transformCoords(srclon, srclat, reqTrans)
        ...
        raise HTTPException(status_code=404, detail="No valid path found (blocked by infinity weights).")
